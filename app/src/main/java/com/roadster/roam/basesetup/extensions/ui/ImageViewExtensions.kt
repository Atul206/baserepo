...
import com.roadster.roam.basesetup.utils.toBitmap
import com.roadster.roam.basesetup.glide.BaseGlideUrl
...
fun createProgressDrawable(context: Context, color:Int) = CircularProgressDrawable(context).apply {
    strokeWidth = context.resources.getDimension(R.dimen.progress_small_stroke_width)
    centerRadius = context.resources.getDimension(R.dimen.progress_small_stroke_width)
    setTint(color)
    start()
...
private fun load(
    ...
    val requestBuilder: RequestBuilder<Drawable> = if (auth && !url.isNullOrBlank()) requestManager.load(BaseGlideUrl(url))
